	   ...
	   while(inputfile>>mot && sortie) 
		...
		if((mot.compare("TBC"))==0)
		{
		  ...
		  while(mot!="]"&&sortie)
		  {
		    ...
		    inputfile>>mot;
		     oss<<T_Total;
// 		    cout<<"cpu_"<<C->CPU_->indice_CPU<<" ("<<i++<<") T_Total: "<<T_Total<<endl;
		  }
...
void GenerateCompositionFile(const char* output,const char* input,const char* newmain, const char* newslice, string indice)
{
 ofstream outputfile(output);/// ios::app pour rajouter en fin du fichier ce qu'on veut ecrire 
  ...
  string line;
  size_t pos;
  string main("main.txt"), slice("slice.txt"), behaviour("slice_behaviour"), carac("main_timing_characs");
  string stringtemp;
  ...
  if(outputfile)
  {
    ...
    if(inputfile)
    {
	  ...
	  {
	      pos=line.find(slice);
	      if(pos<=line.size()) line.replace(pos,slice.size(),newslice);
	      else
	      {
...
//   Affiche_COMPONENTCPU(MesComposantsCPU);
  ...
  while(i<nb_cpu)
  {
    sprintf(dir,"outputfile/slice_cpu_%d.txt",CompoCPU_temp->CPU_->indice_CPU);
    GenerateCpuFile(dir,"slice.txt",CompoCPU_temp);
    if(CompoCPU_temp->CPU_->indice_CPU==0)
    {
      sprintf(dir,"outputfile/main_cpu_%d.txt",CompoCPU_temp->CPU_->indice_CPU);
      GenerateCpuFile(dir,"main.txt",CompoCPU_temp);
    }
//     sprintf(dir,"outputfile/main_cpu_%d.txt",CompoCPU_temp->CPU_->indice_CPU);
//     GenerateCpuFile(dir,"main.txt",CompoCPU_temp);
    i++; 

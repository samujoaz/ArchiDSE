...
uint GenerateCompCpuFile(string input,string output,string includeListe[], string componentListe[] )
{
  ...
  char c[30];
  uint nb_affinity=0,affinity[30];
...
void ChangeAffinity(string Oldcompositionfile, string Newcompositionfile,string component,uint NewAffinity) 
{
  ...
  ofstream outputfile(Newcompositionfile.c_str());   
  if(!outputfile)
  {
    cout<<"impossible de crée le fichier de sortie"<<Newcompositionfile<<endl;
    exit(1);
  }


  ifstream readingfile(Oldcompositionfile.c_str());   
  if(readingfile)
  {
  ...
  {
    cout <<"no such file: "<<Oldcompositionfile<<endl; 
  ...
  ofstream write(Oldcompositionfile.c_str());   
  ifstream read(Newcompositionfile.c_str());

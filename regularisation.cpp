...
{
  uint i=0,o,nb_component=0,numerodossier,indice=0,offset_frequence;
 ...
 // cout << LoadMax << LoadMin <<endl; exit(0);
  /// init variables 
...
//     else TYPE= TYPE2;
  ...
  else
  {
	...
	TYPE = getSpeedTickTimer(compositionfile);//== IS_LOW) TYPE =TYPE3;
      ...
      if(TYPE == IS_HYPER_HIGH) ALGO = SEQUENTIEL;
      else ALGO = MIXTE;
     }
  }
...
case SEQUENTIEL:
	    ...
	    while(copieListeTBC !=NULL && indice<nb_cpu &&  cpuload_cur[0]>LoadMax)/// tant que la liste n'est pas vide ou bien  qu'on pas au cpu_0
	    {
		getAllCpuLoad(nb_cpu,numerodossier++,cpuload_cur,frequence);
		...
		getAllCpuLoad(nb_cpu,numerodossier++,cpuload_cur,frequence);
		if(cpuload_cur[indice]<LoadMin  && copieListeTBC!=NULL && cpuload_cur[0]>LoadMax ) ///si le cpu courant n'est pas assez chargé on lui rajoute des threads
		{
		    o=0;
...
case MIXTE:
	    ...
	    for(uint t=0;t<=SIZE_MIXTE && copieListeTBC !=NULL;t++)
	    {
	    ...
	    if(copieListeTBC==NULL)/// si liste vide 
	    {
		getAllCpuLoad(nb_cpu,numerodossier++,cpuload_cur,frequence);

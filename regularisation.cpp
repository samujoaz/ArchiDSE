...
{
  uint i=0,j=0,o,nb_include=0,nb_affinity =0,numerodossier,indice=0,offset_frequence;
  int frequence[nb_cpu],_nbTBC,NbMaxCpu,TYPE=509,SIZE_MIXTE=0, max,nb_priority;
  char str_indice[100];
...
//     getAllCpuLoad(nb_cpu,numerodossier++,cpuload_cur,frequence);
  ...
  if((_nbTBC/2)+1 < LIMIT && nb_cpu < 3 )
  {
      TYPE = getSpeedTickTimer(compositionfile);
      ALGO = SEQUENTIEL;
  }
  else
  {
...
default:
    cout << "partie de d'algo non implémentée " << endl;  break;
